...
Our analysis platform is primarily based on two forms of Rust intermediate representations: HIR and MIR.
 - [HIR](https://rustc-dev-guide.rust-lang.org/hir.html): An AST-like, type-checked representation that closely follows Rust’s surface syntax while eliminating syntactic sugar.
 - [MIR](https://rustc-dev-guide.rust-lang.org/mir/index.html): A LLVM-IR-like control-flow–oriented representation designed to simplify borrow checking, optimization, and program analysis. Note that MIR is not SSA.
...
- [Place](https://doc.rust-lang.org/beta/nightly-rustc/rustc_middle/mir/struct.Place.html): An abstract memory location in MIR (Mid-level Intermediate Representation). A Place describes where a value is stored or accessed, consisting of a base (such as a local variable or static) and a projection path (e.g., field access, dereference, indexing). 
