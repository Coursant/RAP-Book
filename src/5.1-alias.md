...
Note that this path enumeration strategy can effectively detect the alias relationship between `r` and `x`, for example via the path  
`7-9-10-21-24-11-20-25-19-13-7-9-10-21-24-11-20-25-19-26`.  
In contrast, a straightforward DFS-based traversal fails to detect this alias relationship.

Our analysis is recursive. It therefore starts from the outer SCC and, upon reaching an inner SCC, propagates the accumulated path constraints to test which of the 10 inner-SCC paths are reachable. In other words, the analysis concatenates each reachable subsequence of the inner SCC with the path of the outer SCC.
