...
This module employs call graph segmentation and dataflow analysis for systematic code auditing, as well as utilizes abstract interpretation and contract-based verification to check safety properties. Specifically, it operates on three core pillars:
...
We maintain an annotated fork of Rust standard libraries where every unsafe API is tagged with its required safety properties using tag-std format. For example:
```rust
...
#### 3.2.1 Safety Contract Declaration​
Before verification begins, all unsafe APIs must be explicitly annotated with their required safety properties. These contracts establish the fundamental invariants that callers must guarantee. For our target method `xor_secret_region`, we pre-examine its function body and declare:
...
The similarity between CIS/OTS and safety contracts enables efficient verification. For example, consider the `ptr::offset`'s callsite and contracts:
```
...
count => Var3
```
(1) InBounded (ptr, u32, count) => **InBounded (Var5, u32, Var3)**

* Var5 has OTS: `ValidPtr(u32, var1.1)`. `ValidPtr` is a composite SP, which contains `InBounded(u32, var1.1)`.
```
   Var5: ValidPtr(u32, var11)
=> Var5: InBounded(u32, var11)
=> Var5: InBounded(u32, var11) + Var1.1 >= Var3
