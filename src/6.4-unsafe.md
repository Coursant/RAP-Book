...
RAPx operates at the SP level rather than the UB level:

+ Targets Safety Properties - necessary conditions for UB

+ Performs defect pattern matching per SP rather than per UB type
...
+ Language-Level Casts: Intrinsic operations changing pointer types

+ Function Call Effects: Pointer arithmetic altering address alignment

+ Source Provenance: Untrusted origins without validation

We now examine each category through concrete examples and formal rules.

#### 5.2.1 Intrinsic Type Casts (Language Operations)
...
<pre style="font-family: monospace; background: #f8f8f8; padding: 10px; border-radius: 5px">
    Γ ⊢ statement: left_var = Cast(cast_kind, right_var, τ_dst),
        cast_kind = PtrToPtr,
        right_var.point_to = object_var@(τ_src),
        A_src = align_of(τ_src),
        A_dst = align_of(τ_dst)
    ─────────────────────────────────────────────────────────── 
    Γ ⊢ left_var.point_to = object_var@(τ_src) 
        ▸ Tag(MAYBE_UNALIGNED) if A_src < A_dst
        ▸ Tag(ALIGNED) otherwise
</pre>
...
<pre style="font-family: monospace; background: #f8f8f8; padding: 10px; border-radius: 5px">
    Γ ⊢ statement: left_var = Cast(cast_kind, right_var, τ_dst),
        ...
        A_dst = align_of(τ_dst)
    ─────────────────────────────────────────────────────────── 
    Γ ⊢ left_var.point_to = object_var@(τ_dst) 
        ▸ Tag(MAYBE_UNALIGNED) if A_src < A_dst 
        ▸ Tag(ALIGNED) otherwise 
</pre>

The core principle is comparing the destination type (after casting) with the memory object's original type to verify alignment compatibility. This stems from Rust's memory safety feature where different typed pointers may reference the same memory object. Since alignment of the object is fixed at creation and immutable through casts, our rules must track the original type to ensure valid alignment during pointer transformations.

#### 5.2.2 Function Call Effects (Pointer Arithmetic)
...
**Rule3 (P-Arith)**:
<pre style="font-family: monospace; background: #f8f8f8; padding: 10px; border-radius: 5px">
    Γ ⊢ statement: left_var = Call(func, [base_ptr, offset_val]),
        func ∈ {ptr::byte_add, ptr::byte_offset, ptr::byte_sub},
        base_ptr.point_to = object_var@(τ, offset),
        Δ = abstract_eval(offset_val),
        offset' = offset + Δ
    ─────────────────────────────────────────────────────────── 
    Γ ⊢ left_var.point_to = object_var@(τ, offset'), 
        ▸ Tag(MAYBE_UNALIGNED) if offset' % align_of(τ) ≠ 0 
        ▸ Tag(ALIGNED) otherwise 
</pre>

This rule primarily computes the new offset by deriving it from the original pointer's offset relative to the base address of the memory object. It then employs abstract interpretation to verify whether the resulting offset constitutes an integer multiple of the original object's alignment requirement.

#### 5.2.3 Source Provenance (Untrusted Origins)
...
**Rule4 (U-Source)**:
<pre style="font-family: monospace; background: #f8f8f8; padding: 10px; border-radius: 5px">
    Γ ⊢ statement: left_var = (Copy(external_ptr) | Move(external_ptr))
        source ∈ {FFI, unvalidated_input} 
        ¬∃ alignment_validation(context)
    ─────────────────────────────────────────────────────────── 
    Γ ⊢ object_var = new shadow_var@(unknown),
        external_ptr.point_to = object_var,
        left_var.point_to = object_var,
        ▸ Tag(UNTRUSTED_ALIGN) 
        ▸ Tracked until explicit validation or dereference 
</pre>

This rule creates new memory objects with unknown alignment properties for external pointers. The UNTRUSTED_ALIGN tag persists until explicit validation occurs, ensuring safety through mandatory checks before dangerous operations.
