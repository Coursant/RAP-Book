...
Unlike conventional UB-focused static analyzers, RAPx operates at a more fundamental level:
...
+ The types of MIR statements are limited.

+ Pointer misalignment can only originate from limited fundamental operations.

These two factors indicate that all patterns can be comprehensively modeled in MIR.

Starting from the pointer itself, the scenarios where it may result in an "unaligned" situation are enumerable and can be classified into three aspects: language level casts, influence of function calls, and the trustworthiness of the current pointer source.
...
Type Cast Alignment Rule (T-Cast):
<pre style="font-family: monospace; background: #f8f8f8; padding: 10px; border-radius: 5px">
    Γ ⊢ statement: left_var = Cast(cast_kind, right_var, τ_dst),
        cast_kind = PtrToPtr,
        right_var.point_to = shadow_var@(τ_src),
        A_src = align_of(τ_src),
        A_dst = align_of(τ_dst)
    ─────────────────────────────────────────────────────────── 
    Γ ⊢ left_var.point_to = shadow_var@(τ_src) 
        ▸ Tag(MAYBE_UNALIGNED) if A_src < A_dst
        ▸ Tag(ALIGNED) otherwise
</pre>

Transmute Alignment Rule (T-Transmute): 
<pre style="font-family: monospace; background: #f8f8f8; padding: 10px; border-radius: 5px">
    Γ ⊢ statement: left_var = Cast(cast_kind, right_var, τ_dst),
        cast_kind = Transmute,
        right_var.point_to = shadow_var@(τ_src),
        ...
        A_dst = align_of(τ_dst)
    ─────────────────────────────────────────────────────────── 
    Γ ⊢ left_var.point_to = shadow_var@(τ_dst) 
        ▸ Tag(MAYBE_UNALIGNED) if A_src < A_dst 
        ▸ Tag(ALIGNED) otherwise 
</pre>
...
Untrusted Source Rule (U-Source):
<pre style="font-family: monospace; background: #f8f8f8; padding: 10px; border-radius: 5px">
    Γ ⊢ statement: left_var = (Copy(external_ptr) | Move(external_ptr))
        source ∈ {FFI, unsafe_block, unvalidated_input} 
        ¬∃ alignment_validation(context)
    ─────────────────────────────────────────────────────────── 
    Γ ⊢ left_var.point_to = new shadow_var@(unknown) 
        ▸ Tag(UNTRUSTED_ALIGN) 
